...
def faculty_dashboard_app():
    """Faculty Dashboard with TESDA integration for butterfly production training"""
    display_header("🎓 Faculty Dashboard", "Butterfly Production Training - TESDA Integration", "📚")

    # Check if user has faculty role
    if 'user_role' not in st.session_state or st.session_state.user_role != 'faculty':
        st.warning("⚠️ This section is only available for registered faculty members.")
        st.info("If you are a faculty member, please register with 'faculty' role to access TESDA modules.")
        return

    # Main faculty dashboard content
